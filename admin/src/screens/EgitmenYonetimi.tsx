...
const EgitmenYonetimi = () => {
  const [isOpen, setIsOpen] = useState(false);
  const [alertVisible, setAlertVisible] = useState(false);

  const { data, isError, isLoading } = useQuery({
    queryKey: ["egitmenYonetimi"],
    queryFn: () => ClientBase.get("/api/instructor").then((res) => res.data),
  });

  const deleteMutation = useMutation({
    mutationKey: ["deleteInstructor"],
    mutationFn: (id: string) =>
      ClientBase.delete(`/api/admin/instructor/${id}`, {
        headers: {
          Authorization: `Bearer ${sessionStorage.getItem("token")}`,
        },
      }),
    onSuccess: (data, variables) => {
      queryClient.setQueryData(["egitmenYonetimi"], (old: any) => [
        ...old.filter((egitmen: any) => egitmen._id !== variables),
      ]);
    },
    onError: (e) => {
      console.log(e);
  ...
  const mutation = useMutation({
    ...
    mutationFn: (form: any) =>
      ClientBase.post("/api/admin/instructor", form, {
        headers: {
          Authorization: `Bearer ${sessionStorage.getItem("token")}`,
        },
      }),
  ...
  const handleSubmit = (event: any) => {
    ...
    uploadImage(formData.get("egitmenResmi"), "instructors").then(
      (url: string) => {
        mutation.mutate({
          name: formData.get("egitmenAdi"),
          description: formData.get("egitmenAciklamasi"),
          createdBy: formData.get("olusturanKisi"),
          imageURL: url,
        });
        setIsOpen(false);
      }
  ...
  return (
    <div>
      <Header pageTitle="Eğitmen Yönetimi" />
      <Modal
        height="80%"
        title="Eğitmen Ekle"
        isOpen={isOpen}
        ...
        <div className="w-full flex justify-center mt-10">
          ...
          >
            ...
            <textarea
              ...
              placeholder="Eğitmen Açıklaması"
            />
            <input
              autoComplete="off"
              name="olusturanKisi"
      ...
      <Alert
        alertType="success"
        alertDescription="Eğitmen Başarıyla Eklendi!"
      ...
      {isLoading ? (
        ...
        <Container>
          <div className="grid grid-cols-4 gap-10">
            {data.map((egitmen: any) => (
              <button
                onClick={() =>
                  confirm("Silmek istediğinize emin misiniz?") &&
                  deleteMutation.mutate(egitmen._id)
                }
                key={egitmen._id}
              ...
              >
                ...
                />
                <h2 className="font-semibold text-sm">{egitmen.name}</h2>
        ...
        <h1 className="text-center">Eğitmen İçeriği Bulunamadı!</h1>
      )}
    </div>
  );
};

export default EgitmenYonetimi;
